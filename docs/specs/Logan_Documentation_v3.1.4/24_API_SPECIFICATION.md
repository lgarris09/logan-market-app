...
**Target design vs. what V3.1.4 actually shipped:** this whole document describes the target API design (JWT auth, `{data, meta, error}` envelope, `hit_quality_score`/`user_value_score`/`momentum_score` fields) — none of which exists yet. The real `GET /v1/opportunities` shipped in V3.1.4 BATCH-4 (`backend/app/opportunities.py`) is unauthenticated (no auth system exists at all — see `27_SECURITY_PRIVACY_COMPLIANCE.md`), returns a bare `{schema_version, items, generated_at}` object with no envelope, and each item is `logan_feed.py`'s `FeedItem` shape (`event_id, entity_id, display_name, category, ticker, domain, delivered_item, rank, confidence_score, confidence_label, connected_event_ids` — no `hit_quality_score`/`user_value_score`/`momentum_score`, and never `internal_rank_score`, ADR-029). It accepts one optional `category` query parameter. See `mobile/types/loganFeed.ts`'s `OpportunitiesResponse`/`FeedItem` for the authoritative real shape and `backend/tests/test_opportunities_api.py` for its contract tests.
