...
**Version:** 3.1.4
*Last updated: 2026-08-21 (Sprint 3.6.6 close-out, ADR-049) — verified against the actual repository, not reconstructed.*
...
| TriggerEvent registry | PARTIALLY BUILT (Sprint 3.6.6, extended 3.6.6D) — three trigger codes, minimal contract | `logan_core/contracts/trigger.py` (a minimal `TriggerEvent`, not the full ~60-field `TRIGGER_EVENT_FRAMEWORK.md` contract — no revision/dedup model, `domain_impacts`, `lifecycle_effect`, `seasonal_context`, `causal_relationship`, `provider_disagreement_state`, `notification_eligibility`, etc.) and `logan_core/trigger_detection/stocks.py` implement `STOCK_EARNINGS_BEAT`, `STOCK_EARNINGS_MISS`, and `STOCK_EARNINGS_IN_LINE` from `TRIGGER_REGISTRY_STOCKS.md`, all three sharing the same `actual_eps`/`consensus_eps` provider fields, mutually exclusive by fire-condition construction. Every other stocks code (`STOCK_EARNINGS_QUALITY_WARNING`, `STOCK_GUIDANCE_RAISED`/`LOWERED`, `STOCK_OPTIONS_FLOW_SURGE`, etc. — each needing data FMP's earnings endpoint doesn't return) and every other domain's registry remain SPECIFIED — NOT IMPLEMENTED, unchanged from OD-009 (V3.1.4 BATCH-3). See docs/DECISIONS.md ADR-042/ADR-045, which partially — not fully — supersede OD-009. **Known duplication, not yet fixed:** every threshold and `confidence_contribution` value (5.0/5.0/2.0 and 0.22/0.20/0.0) is a typed Python constant in `trigger_detection/stocks.py`, hand-copied from `TRIGGER_REGISTRY_STOCKS.md`'s markdown table with a comment citing it — there is no machine-readable registry loader, so the doc and the code are two independently-edited copies of the same numbers with no automated drift check between them. Verified consistent as of Sprint 3.6.6D (2026-08-18). Acceptable at 3 trigger codes; convert to a single machine-readable source of truth (the registry loaded at runtime, not re-typed per evaluator) before trigger volume grows much further — explicitly deferred, not forgotten, per owner decision |
