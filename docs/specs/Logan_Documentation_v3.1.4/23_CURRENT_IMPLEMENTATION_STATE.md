...
| Domain Receptors | SIMULATED ONLY for the live backend path; one real provider implementation exists but is not wired in (Sprint 3.6.6B) | `logan_core/receptors/simulated.py` — every domain's live backend path (`backend/app/logan_feed.py`) remains simulated-only, NVDA included. `receptors/providers/`: `EarningsProvider` Protocol, `FixtureEarningsProvider` (explicitly labeled non-live, used by tests), and `FmpEarningsProvider` (Sprint 3.6.6B — a real Financial Modeling Prep implementation, contract-tested against `httpx.MockTransport`, not yet run against the live FMP API pending an owner-provided `FMP_API_KEY`). `receptors/stocks_earnings.py` maps provider data into `RawSignal`. See docs/DECISIONS.md ADR-042/ADR-043 |
