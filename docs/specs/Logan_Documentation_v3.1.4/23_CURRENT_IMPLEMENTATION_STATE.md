...
**Version:** 3.1.4
*Last updated: 2026-08-21 (Sprint 3.6.7 Block 1, ADR-050/051) — verified against the actual repository, not reconstructed.*
...
**Known limitation carried into Sprint 3.6.7 Block 2, not resolved this pass:** true multi-signal-type
convergence for one entity within one poll — the exact precondition for implementing
`STOCK_CONVERGENCE_MULTI_SOURCE` (`TRIGGER_REGISTRY_STOCKS.md`, SPECIFIED — NOT IMPLEMENTED) — remains
unbuilt; `WorldModel`'s dedup/merge model needs to widen beyond one `(entity_id, signal_type)` key before
either that trigger code or full live-endpoint wiring for the Block 1 signals can be done correctly. See
`SESSION_NOTES.md`'s Sprint 3.6.7 Block 2 starting-point section.

See docs/DECISIONS.md ADR-050 (Personal-route authority rule) and ADR-051 (generalized stock-signal
architecture) for the full decision records.
