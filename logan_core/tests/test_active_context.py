...
def test_time_of_day_buckets():
    ...
    morning = builder.build(
        user_id="demo_user", now=datetime(2026, 8, 5, 7, 0, tzinfo=timezone.utc)
    )
    night = builder.build(
        user_id="demo_user", now=datetime(2026, 8, 5, 23, 0, tzinfo=timezone.utc)
    )
...
def test_current_question_and_recent_activity_carried_through():
    ...
    activity = [
        ActivityRecord(activity_type="view", detail="viewed NVDA", occurred_at=now)
    ]
    builder = ActiveContextBuilder()
    context = builder.build(
        user_id="demo_user",
        current_question="what about NVDA?",
