...
def test_valid_signal_normalizes_successfully():
    raw = _raw({"entity_id": "NVDA", "entity_type": "ticker", "signal_type": "earnings_signal", "value": "beat"})
...
def test_missing_entity_id_is_rejected():
    raw = _raw({"entity_type": "ticker", "signal_type": "earnings_signal", "value": "beat"})
    with pytest.raises(NormalizationError):
        Normalizer().normalize(raw)


def test_missing_value_is_rejected():
    raw = _raw({"entity_id": "NVDA", "entity_type": "ticker", "signal_type": "earnings_signal"})
    with pytest.raises(NormalizationError):
        Normalizer().normalize(raw)


def test_unregistered_signal_type_for_domain_is_rejected():
    raw = _raw({"entity_id": "NVDA", "entity_type": "ticker", "signal_type": "odds_move", "value": "x"}, domain="stocks")
    with pytest.raises(NormalizationError):
        Normalizer().normalize(raw)


def test_signal_type_valid_in_its_own_domain():
    raw = _raw({"entity_id": "LAL", "entity_type": "team", "signal_type": "odds_move", "value": "x"}, domain="sports")
    normalized = Normalizer().normalize(raw)
