...
def test_first_load_is_notification_silent():
    reset_pipeline_state()
    items, _now, _alert_event_ids, _provider_degraded = _run_feed_pipeline(
...
def test_repeated_request_does_not_re_notify():
    ...
    """
    reset_pipeline_state()
    first_items, _now, _alert_event_ids, _provider_degraded = _run_feed_pipeline(
        LOCAL_FOUNDER_USER_ID
    )
    second_items, _now, _alert_event_ids, _provider_degraded = _run_feed_pipeline(
        LOCAL_FOUNDER_USER_ID
    )
